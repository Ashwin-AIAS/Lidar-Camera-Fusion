...
###################################################################################################################################
#kitty dataset dataloader
def img2label_paths_kitty(img_paths):
    """Generates label file paths from corresponding image file paths by replacing `/images/` with `/labels/` and
    extension with `.txt`.
    """
    label_paths = []
    ...
    for img_path in img_paths:
        if "kitty" in img_path.lower():  # Check if 'kitty' is in the image path
            sa, sb = f"{os.sep}images{os.sep}", f"{os.sep}annotations{os.sep}"
            label_path = sb.join(img_path.rsplit(sa, 1)).rsplit(".", 1)[0] + ".png" + ".json"
        elif "audi" in img_path.lower():  # Check if 'audi' is in the image path
            sa, sb = f"{os.sep}images{os.sep}", f"{os.sep}annotations{os.sep}"
            label_path = sb.join(img_path.rsplit(sa, 1)).rsplit(".", 1)[0].replace("_camera_", "_label3D_") + ".json"
        elif "tumv" in img_path.lower():  # Check if 'kitty' is in the image path
            sa, sb = f"{os.sep}images{os.sep}", f"{os.sep}annotations{os.sep}"
            label_path = sb.join(img_path.rsplit(sa, 1)).rsplit(".", 1)[0] + ".json"
        elif "cityscape" in img_path.lower():
            sa, sb = f"{os.sep}images{os.sep}", f"{os.sep}annotations{os.sep}"
            label_path = sb.join(img_path.rsplit(sa, 1)).rsplit(".", 1)[0].replace("_leftImg8bit_", "gtFine_polygons_") + ".json"
        # else:
        #     raise ValueError(f"Unknown dataset type in image path: {img_path}. Expected 'kitty' or 'audi'.")
        
        label_paths.append(label_path)

    return label_paths
#####################################################################################################################################
#audi dataset dataloader
# def img2label_paths_audi(img_paths):
#     """Generates label file paths from corresponding image file paths by replacing `/images/` with `/labels/` and
#     extension with `.txt`.
#     """

#     sa, sb = f"{os.sep}images{os.sep}", f"{os.sep}annotations{os.sep}"  # /images/, /labels/ substrings
#     return [sb.join(x.rsplit(sa, 1)).rsplit(".", 1)[0].replace("_camera_", "_label3D_")+".json" for x in img_paths]
#####################################################################################################################################
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(
        ...
        try:
            f = []  # image files
            for p in path if isinstance(path, list) else [path]:        #each image in folder is taken
        ...
        self.label_files = img2label_paths_kitty(self.im_files)  # labels
        # self.label_files = img2label_paths_audi(self.im_files)
        # if any("audi" in str(p).lower() for p in f):  
        #     # If any file path contains "kitty"
        #     self.label_files = img2label_paths_audi(self.im_files)
        # elif any("kitty" in str(p).lower() for p in f):  
        #     # If any file path contains "audi"
        #     self.label_files = img2label_paths_kitty(self.im_files)
        ...
        self.labels = list(labels)                  #important, this is label
        self.shapes = np.array(shapes)
        self.im_files = list(cache.keys())  # update
        #self.label_files = img2label_paths(cache.keys())  # update
        self.label_files = img2label_paths_kitty(cache.keys())  # update
    ...
    def cache_labels(self, path=Path("./labels.cache"), prefix=""):
        ...
        with Pool(NUM_THREADS) as pool:
            ...
            # )
            pbar = tqdm(
                pool.imap(verify_image_label_kitty, zip(self.im_files, self.label_files, repeat(prefix))),
                desc=desc,
                total=len(self.im_files),
                bar_format=TQDM_BAR_FORMAT,
            )
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if labels.size:  # normalized xywh to pixel xyxy format
                labels[:, 1:] = xywhn2xyxy(labels[:, 1:], ratio[0] * w, ratio[1] * h, padw=pad[0], padh=pad[1])#corresponds to x,y,w,h
...
##################################################################################################################################
#Kitty dataset dataloader
def verify_image_label_kitty(args):
    """Verifies a single image-label pair, ensuring image format, size, and legal label values."""
    im_file, lb_file, prefix = args
    nm, nf, ne, nc, msg, segments = 0, 0, 0, 0, "", []  # number (missing, found, empty, corrupt), message, segments
    try:
        ...
        if os.path.isfile(lb_file):
            nf = 1  # label found
            # lb=kitty_conversion(lb_file)
            # lb=audi_conversion(lb_file)
            if "audi" in lb_file.lower():
                lb = audi_conversion(lb_file)  # Process with kitty conversion
            elif "kitty" in lb_file.lower():
                lb = kitty_conversion(lb_file)  # Process with audi conversion
            elif "tum" in lb_file.lower():
                lb = tum_conversion(lb_file)
            #print(lb)
            if any(len(x) > 6 for x in lb):  # is segment
                classes = np.array([x[0] for x in lb], dtype=np.float32)
                segments = [np.array(x[1:], dtype=np.float32).reshape(-1, 2) for x in lb]  # (cls, xy1...)
...
######################################################################################################################################
def kitty_conversion(json_file):
    class_mapping = {
        'car': 0,
        'pedestrian': 1,
        'person sitting' : 1,
    ...
    for obj in data['objects']:
        ...
        yolo_lines.append([class_id, x_center, y_center, width, height])
        #print(yolo_lines)
    return np.array(yolo_lines, dtype=np.float32)
###################################################################################################################################

def audi_conversion(json_file):
    class_mapping = {
        "Car": 0,
        "Pedestrian": 1,
        "Cyclist": 2,
    ...
    for box_key, box_value in data.items():
        class_name = box_value.get("class", "unknown")
        class_id = class_mapping.get(class_name, -1)

        if class_id == -1:
            continue
        x_min, y_min, x_max, y_max = box_value["2d_bbox"]
        ...
        bbox_center_x = (x_min + x_max) / 2.0 / image_width
        bbox_center_y = (y_min + y_max) / 2.0 / image_height
        bbox_width = (x_max - x_min) / image_width
        bbox_height = (y_max - y_min) / image_height
        #Extract 2D bounding box

        # Format: class_id center_x center_y width height
        # yolo_line = f"{class_id} {bbox_center_x:.6f} {bbox_center_y:.6f} {bbox_width:.6f} {bbox_height:.6f}"
        # yolo_lines.append(yolo_line)
        yolo_lines.append([class_id, bbox_center_x, bbox_center_y, bbox_width, bbox_height])
        #print(yolo_lines)
    return np.array(yolo_lines, dtype=np.float32)

#######################################################################################################################################
def tum_conversion(json_path):
    CLASS_MAPPING = {
        'CAR': 0,
        'PEDESTRIAN': 1,
        'CYCLIST': 2,
    ...
    for obj in data.get('labels', []):
        ...
        #class_id = CLASS_MAPPING.get(class_name.lower(), CLASS_MAPPING['car'])  # Default to 'car'
        class_id = CLASS_MAPPING.get(class_name) 

        box = obj.get('box3d_projected', {})
        if not box:
            continue
                
        # x_min = max(min(box.get('bottom_left_front', [0, 0])[0], box.get('bottom_right_front', [0, 0])[0]),0)
        # y_min = max(min(box.get('bottom_left_front', [0, 0])[1], box.get('top_left_front', [0, 0])[1]),0)
        # x_max = min(max(box.get('top_right_front', [0, 0])[0], box.get('bottom_right_front', [0, 0])[0]),1)
        # y_max = min(max(box.get('top_right_front', [0, 0])[1], box.get('bottom_right_front', [0, 0])[1]),1)
        x_min = max(min(box.get('bottom_left_front', [0, 0])[0], box.get('top_left_front', [0, 0])[0],box.get('bottom_left_back', [0, 0])[0],box.get('top_left_back', [0, 0])[0]),0)
        #y_min = max(min(box.get('bottom_left_front', [0, 0])[1], box.get('top_left_front', [0, 0])[1]),0)
        y_min = max(min(box.get('bottom_left_front', [0, 0])[1], box.get('top_left_front', [0, 0])[1],box.get('bottom_left_back', [0, 0])[1],box.get('top_left_back', [0, 0])[1]),0)
        x_max = min(max(box.get('bottom_right_front', [0, 0])[0], box.get('top_right_front', [0, 0])[0],box.get('bottom_right_back', [0, 0])[0],box.get('top_right_back', [0, 0])[0]),1)
        #x_max = min(max(box.get('top_right_front', [0, 0])[0], box.get('bottom_right_front', [0, 0])[0]),1)
        #y_max = min(max(box.get('top_right_front', [0, 0])[1], box.get('bottom_right_front', [0, 0])[1]),1)
        y_max = min(max(box.get('bottom_right_front', [0, 0])[1], box.get('top_right_front', [0, 0])[1],box.get('bottom_right_back', [0, 0])[1],box.get('top_right_back', [0, 0])[1]),1)

                # Normalize coordinates for YOLO format
        x_center = ((x_min + x_max) / 2)# / DEFAULT_WIDTH
        y_center = ((y_min + y_max) / 2)# / DEFAULT_HEIGHT
        width = abs(x_max - x_min)# / DEFAULT_WIDTH
        height = abs(y_max - y_min)# / DEFAULT_HEIGHT
